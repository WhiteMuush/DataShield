...
doctor: ## Full setup diagnosis: toolchain, env, Docker, database, Prisma
	@ok=0; warn=0; bad=0; dkr=0; \
	fx_env=0; fx_auth=0; fx_enc=0; fx_authurl=0; fx_cron=0; fx_install=0; fx_generate=0; fx_dbup=0; fx_migrate=0; \
	pass() { echo "  [OK]   $$1"; ok=$$((ok+1)); }; \
	wrn()  { echo "  [WARN] $$1"; warn=$$((warn+1)); }; \
	err()  { echo "  [FAIL] $$1"; bad=$$((bad+1)); }; \
	getv() { grep -E "^$$1=" .env.local 2>/dev/null | head -1 | cut -d= -f2-; }; \
	setval() { if grep -qE "^$$1=" .env.local 2>/dev/null; then sed -i "s|^$$1=.*|$$1=$$2|" .env.local; else printf '%s=%s\n' "$$1" "$$2" >> .env.local; fi; }; \
	...
	if [ ! -f .env.local ]; then err ".env.local missing (run 'make env')"; fx_env=1; \
	else \
		...
		if [ -n "$$(getv AUTH_URL)" ]; then pass "AUTH_URL set"; else wrn "AUTH_URL empty (defaults to http://localhost:3000)"; fx_authurl=1; fi; \
		if [ -n "$$(getv CRON_SECRET)" ]; then pass "CRON_SECRET set"; else wrn "CRON_SECRET empty (scheduler endpoint /api/cron returns 503)"; fx_cron=1; fi; \
	...
	if [ $$((errfix+warnfix)) -gt 0 ] && [ -t 0 ]; then \
		...
		printf "Fix now? [e]rrors only / [a]ll / [n]o: "; read ans; \
	fi; \
	do_err=0; do_warn=0; \
	case "$$ans" in e|E) do_err=1;; a|A) do_err=1; do_warn=1;; esac; \
	applied=0; \
	if [ "$$do_err" = "1" ]; then \
		if [ "$$fx_env" = "1" ]; then cp .env.example .env.local; setval AUTH_SECRET "$$(openssl rand -base64 32)"; setval DIRECTORY_ENCRYPTION_KEY "$$(openssl rand -base64 32)"; echo "fixed: created .env.local with generated secrets"; applied=1; \
		else \
	...
	if [ "$$do_warn" = "1" ]; then \
		...
		if [ "$$fx_dbup" = "1" ]; then npm run db:up; echo "waiting for db to become healthy"; i=0; while [ "$$(docker inspect -f '{{.State.Health.Status}}' datashield-db 2>/dev/null)" != "healthy" ] && [ $$i -lt 60 ]; do i=$$((i+1)); sleep 1; done; applied=1; fi; \
		if [ "$$fx_migrate" = "1" ]; then npx prisma migrate deploy; applied=1; fi; \
	fi; \
	if [ "$$applied" = "1" ]; then echo ""; echo "Fixes applied. Re-run 'make doctor' to verify."; exit 0; fi; \
	[ "$$bad" -eq 0 ]
